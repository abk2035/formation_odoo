...
class BankTransactions(models.Model):
    _name = "dm.bank.transactions"
    _description = "DM Bank Transactions Model"

    name = fields.Many2one("res.partner", string="Customer", required=True)
    bank_id = fields.Many2one("dm.bank", string="Bank", required=True,
                              default=lambda lm:lm.env.user.bank_id.id
                              )
    ...
    def transaction_done(self):
        ...
        self.state = 'done'
        self.env.ref("deposit_management.bank_transaction_done_email_template").send_mail(self.id, force_send=False)